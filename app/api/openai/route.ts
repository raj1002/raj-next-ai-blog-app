...
export async function POST(request: Request, response: any) {
    try {
        ...
        const aiResponse: AxiosResponse<CreateChatCompletionResponse, any> = await openai.createChatCompletion({
            model: "gpt-3.5-turbo",
            messages: [{role: "user", content: `Create a 3 line blog post with html tahs based on this title: ${title}`},
        {
            role: "system",
        ...
        return NextResponse.json({
            content: aiResponse.data.choices[0].message?.content}, { status: 200 });
    } catch (error) {
